// combat-service/src/controller/CombatController.ts
import { Request, Response } from 'express';
import { AppDataSource } from '../config/database.config';
import { Duel } from '../entity/Duel';
import { DuelAction } from '../entity/DuelAction';
import { AuthRequest } from '../middleware/auth';
import { CharacterSync } from '../services/CharacterSync';
...
export class CombatController {
    ...
    static async attack(req: AuthRequest, res: Response): Promise<void> {
        try {
            ...
            if (lastAction && (Date.now() - lastAction.executedAt.getTime()) < 1000) {
                res.status(429).json({ message: 'Attack available every 1 second' });
                return;
            }
            ...
            res.json({ message: 'Attack executed', damage });
        } catch (error) {
    ...
    static async cast(req: AuthRequest, res: Response): Promise<void> {
        try {
            ...
            if (lastAction && (Date.now() - lastAction.executedAt.getTime()) < 2000) {
                res.status(429).json({ message: 'Cast available every 2 seconds' });
                return;
            }
            ...
            res.json({ message: 'Cast executed', damage });
        } catch (error) {
    ...
    static async heal(req: AuthRequest, res: Response): Promise<void> {
        try {
            ...
            if (lastAction && (Date.now() - lastAction.executedAt.getTime()) < 2000) {
                res.status(429).json({ message: 'Heal available every 2 seconds' });
                return;
            }
            ...
            characterStats.health = Math.min(characterStats.health + healing, 100); // Cap at 100
            ...
            res.json({ message: 'Heal executed', healing });
        } catch (error) {
    ...
    private static async validateDuel(req: AuthRequest): Promise<Duel | null> {
        ...
        if (!duel) {
            req.res!.status(404).json({ message: 'Duel not found or ended' });
            return null;
        }
        if (req.user!.id !== duel.challengerId && req.user!.id !== duel.opponentId) {
            req.res!.status(403).json({ message: 'Not a duel participant' });
            return null;
        }
        return duel;
    }
